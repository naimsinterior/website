...
const feedbackFormSchema = z.object({
  ...
  designProcessSatisfaction: z.number().min(1).max(5),
  needsUnderstanding: z.enum(["yes", "no", "somewhat"], { required_error: "Please select an option."}),
  finalDesignRating: z.number().min(1).max(5),
  designerComfort: z.enum(["yes", "no", "neutral"], { required_error: "Please select an option."}),
  designFlexibility: z.enum(["yes", "no", "somewhat"], { required_error: "Please select an option."}),
  
  onTimeHandover: z.enum(["yes", "no", "delayed"], { required_error: "Please select an option."}),
  qualityRating: z.number().min(1).max(5),
  communicationSatisfaction: z.number().min(1).max(5),
  fairPricing: z.enum(["yes", "no", "neutral"], { required_error: "Please select an option."}),
  valueForMoney: z.enum(["yes", "no", "neutral"], { required_error: "Please select an option."}),
  challengesFaced: z.string().optional(),

  wouldRecommend: z.enum(["yes", "no", "maybe"], { required_error: "Please select an option."}),
  referralLikelihood: z.number().min(1).max(10),
  howHeard: z.string().optional(),
  experienceInOneWord: z.string().optional(),
  favoritePart: z.string().optional(),
  workWithUsAgain: z.enum(["yes", "no", "maybe"], { required_error: "Please select an option."}),
...
const STEPS = [
    {
        ...
        fields: ["whyChoose", "designProcessSatisfaction", "needsUnderstanding", "finalDesignRating", "designerComfort", "designFlexibility"]
    },
    {
        title: "Execution & Quality",
        fields: ["onTimeHandover", "qualityRating", "communicationSatisfaction", "fairPricing", "valueForMoney", "challengesFaced"]
    },
    {
        title: "Overall Experience & Final Thoughts",
        fields: ["wouldRecommend", "referralLikelihood", "howHeard", "experienceInOneWord", "favoritePart", "workWithUsAgain", "improvementSuggestion", "additionalComments"]
    },
     {
...
export default function FeedbackPage() {
    ...
    const handleNext = async () => {
        ...
        const output = await form.trigger(fields as FieldName<FeedbackFormValues>[], { shouldFocus: true });
    ...
    if (isSubmitted) {
        return (
             <div className="container mx-auto px-4 py-16 md:px-6 md:py-24 flex items-center justify-center min-h-[60vh]">
                <div className="text-center">
                    <MessageSquarePlus className="mx-auto h-12 w-12 text-primary" />
                    <h1 className="mt-4 font-headline text-4xl md:text-5xl">Thank You!</h1>
